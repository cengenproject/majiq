...
outdir='/gpfs/ycga/project/ysm/hammarlund/aw853/majiq/outs/2024-03-04_outs'
...
ref_gtf=$references_dir/$WSversion/c_elegans.PRJNA13758.$WSversion.canonical_geneset.gtf
...
gtf_prepared='intermediates/240304_novel_filt_nogene.gtf'
#ref_gff='intermediates/240301_novel_filt.gff3'
...
/home/aw853/bin/gtf2gff3.pl \
    --cfg $references_dir/gtf2gff3.cfg \
    $gtf_prepared \
    > $ref_gff
...
# note: need to set new array values
set +u
  ...
  for (( j = $i+1; j < ${#all_neurs[@]}; j++))
    ...
    #   --output-type {voila,tsv,all} Specify the type(s) of output files to produce [Default: all]
		...
		majiq deltapsi \
		  ...
		  --nproc $SLURM_CPUS_PER_TASK \
		  --output-type all \
		  --logger $outdir/logs/deltapsi.log
